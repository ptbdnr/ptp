...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if stream:
                # Use a simple sentence with clear words
                sentence = "This is a test message to verify streaming functionality."
                words = sentence.split()
                
                # Stream each word one at a time
                for word in words:
                    # Send the word with a trailing space
                    chunk = word + " "
                    logger.info(f"[Backend] Sending: '{chunk}'")
                    await websocket.send_text(chunk)
                
                # Send end marker
                logger.info("[Backend] Sending end marker")
                await websocket.send_json({"type": "end"})
            else:
                # Send complete response without streaming
                message = "This is a test message to verify streaming functionality."
                logger.info(f"[Backend] Sending complete message: '{message}'")
